...
export default function OnlineLecture({navigation, route}) {
  ...
  const [recDaysID, setRecDaysID] = useState({});
  ...
  useEffect(async () => {
    ...
    try {
      ...
      setUrl(LiveClass.url);
      setDescription(LiveClass.name);
      setDate(LiveClass.date);
      setTime(LiveClass.time);

      setTeacher(LiveClass.teacher);
      setv(LiveClass.__v);
      setid(LiveClass._id);

      LiveClass.days.map(day => {
        recDays[day.name] = day.checked;
        recDaysID[day.name] = day._id;
      });
  ...
  const handleSaveClass = async () => {
    ...
    try {
      ...
      alert('Cannot Add this Class!' + err);
    }
    hideLoadingScreen();
  };
