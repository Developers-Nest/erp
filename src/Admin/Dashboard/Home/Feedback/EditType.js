...
const EditType = ({route, navigation}) => {
  //theming
  ...
  //data to be sent
  const [feedbackfor, setfeedbackfor] = useState(route.params.feedbackfor);
  const [status, setstatus] = useState(route.params.status);
  const [feedbacktype, setfeedbacktype] = useState(route.params.feedbacktype);
  ...
  let handleSubmit = async () => {
    showLoadingScreen();
    try {
      let slug = `/feedback/type/${route.params.id}`;
      ...
      console.log('edit type data', data);
      let res = await patch(slug, data, token);
      if (res.error) {
        alert(res.error);
      } else if (res._id) {
        alert('Feedback Type Updated!!');
        navigation.replace('TypeList');
      }
    } catch (err) {
